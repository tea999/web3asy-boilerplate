...
const downloadUrl = process.env.DOWNLOADURL || "https://custonomy.io/download";
...
function getEmailParams(template: string | number, toUser: string, content: any | null, header = "https://api.custonomy.io:8000") {
  switch (template) {
    ...
    case EMAIL_TEMPLATE.SOCIAL_SIGNUP:
        // Object.assign(emailtmpl, email[template]);
      // console.log({toUser});
      // emailtmpl.Message.Body.Html.Data = emailtmpl.Message.Body.Html.Data.replace('${name}', toUser.firstName + ' ' + toUser.lastName).replace('${newpassword}', content.password);
      ...
      // emailTempl.Message.Body.Html.Data = mailMerge(emailBody[template], data);
      // emailTempl.Message.Subject.Data = subject[template];
      // emailTempl.Destination.ToAddresses.push(toUser.email);
      ...
      // console.log('email', _emTmpl.Destination.ToAddresses);

      // console.log(JSON.stringify(emailtmpl, null, 2))
      return _emTmpl;
      break;
  }
}
